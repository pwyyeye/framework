...
public class Base64 {
	...
	private static final char[] LOOKUP_BASE64ALPHABET = new char[64];
	...
	static {
		...
		for (int i = 0; i <= 25; i++) {
			LOOKUP_BASE64ALPHABET[i] = ((char) (65 + i));
		}

		int i = 26;
		for (int j = 0; i <= 51; j++) {
			LOOKUP_BASE64ALPHABET[i] = ((char) (97 + j));
		...
		i = 52;
		for (int j = 0; i <= 61; j++) {
			LOOKUP_BASE64ALPHABET[i] = ((char) (48 + j));

			i++;
		}

		LOOKUP_BASE64ALPHABET[62] = '+';
		LOOKUP_BASE64ALPHABET[63] = '/';
	}
	...
	public String encode(byte[] binaryData) {
		...
		for (int line = 0; line < numberLines - 1; line++) {
			for (int quartet = 0; quartet < 19; quartet++) {
				...
				encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[val1];
				encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(val2 | k << 4)];

				encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(l << 2 | val3)];

				encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(b3 & 0x3F)];
		...
		if (fewerThan24bits == 8) {
			...
			encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[val1];
			encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(k << 4)];
			encodedData[(encodedIndex++)] = '=';
			encodedData[(encodedIndex++)] = '=';
			...
			encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[val1];
			encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(val2 | k << 4)];
			encodedData[(encodedIndex++)] = LOOKUP_BASE64ALPHABET[(l << 2)];
			encodedData[(encodedIndex++)] = '=';
		}
