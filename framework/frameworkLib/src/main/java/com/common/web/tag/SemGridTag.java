...
import com.common.web.utils.SemWebAppConstants;
import com.common.web.utils.SemWebAppUtils;

public class SemGridTag extends TagSupport {
	...
	public int doEndTag() throws JspException {
		...
		try {
					...
					while (iter.hasNext()) {
						...
						if (content.length == 3) {// 带有超级链接
							...
							if(content[2].startsWith("_")){//user defined renderer
								out
								.println(",renderer:function link(value,row,rec){return "+content[2]+";}");
							}else if (content[2].indexOf("%") != -1) {
			...
			out.println("}");
            out.println("var gridHeadField=new Ext.form.Hidden({name:'excelHead'});");
