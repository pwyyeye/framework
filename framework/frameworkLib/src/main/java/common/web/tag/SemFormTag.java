...
public class SemFormTag extends TagSupport {
	public Log logger = LogFactory.getLog(this.getClass());

	protected String recordLabel = "";

	protected String action = null; // 后台处理的 item action

	protected String headAction = null; // 后台处理的head Form Action

	// protected String headSaveAction=null;//后台处理head保存的Action

	protected String gridBody = null;// 定义表格中要显示的表头及内容：eg：id:,name:角色名称,module:模块名,moduleID,empID:姓名

	protected String formWidth = "500";// 默认弹出框宽度为400；
	...
	protected String headFormString = null; // head form输入框

	protected String headField = null; // head form 中对应的字段名

	protected String headCustonButton = null; // head form的自定义BUTTON

	protected String headIdParamName = SemWebAppConstants.FORM_TAG_HEAD_ID;

	protected String rightString = null;// 按钮权限

	protected String gridTitle = "Detail Info";

	protected String headTitle = "Head Info";
	...
	protected String hideHeadSaveButton = "N";// 隐藏保存按钮

	protected String headSubmitContent = null;
	...
	public String getHeadSubmitContent() {
		return headSubmitContent;
	}

	public void setHeadSubmitContent(String headSubmitContent) {
		this.headSubmitContent = headSubmitContent;
	}

	public String getHideHeadSaveButton() {
		return hideHeadSaveButton;
	}

	public void setHideHeadSaveButton(String hideHeadSaveButton) {
		this.hideHeadSaveButton = hideHeadSaveButton;
	}

	private boolean isHideHeadSaveButton() {
		return this.hideHeadSaveButton != null
				&& "Y".equalsIgnoreCase(hideHeadSaveButton);
	}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			String headId = (String) pageContext.getAttribute(
					this.headIdParamName, PageContext.REQUEST_SCOPE);
			out.println(" var currentID='';");
			if (SemWebAppUtils.isNotEmpty(headId)) {
				out.println("currentID='" + headId + "';");
			}
			...
			if (otherSearchForm) {// 单独的searchForm窗口
				out
						.println("var searForm = new Ext.FormPanel({labelSeparator : '：',frame:true,border:false,");
				out.println("items :[" + searchFormString + "],");
				out
						.println("buttons:[{text : '关闭',handler : function(){win2.hide();}},{text : '提交',handler : submitSearch}]});");
			}
			out.println(" var viewport = new Ext.Viewport({layout:'border',");
			out.println("items:[");
			out
					.println("{region:'north',contentEl: 'totalForm',autoHeight: true,autoWidth:true,collapsed:false,maxSize: 200,collapsible: true,floatable: false,title:'"
							+ headTitle + "', margins:'0 0 0 0'}, ");
			out
					.println("{region:'center',contentEl: 'centertable',title:'"
							+ gridTitle
							+ "',split:true,collapsible: true,collapsed:false,margins:'0 0 0 0'}");
			out.println(" ]});");

			out.println("var store = new Ext.data.Store({");
			out.println("autoLoad :false,");
			out.println("reader : new Ext.data.JsonReader({");
			...
			out.println("})");
			out.println("});");
			out.println("var toolbar = new Ext.Toolbar([");
			boolean first = true;
			...
			out.println("]);");
			out.println("var combo = new Ext.form.ComboBox({");
			...
			out.println("]);");
			out.println("var grid = new Ext.grid.LockingGridPanel({");
			/**
			 * if (gridTitle != null) { out.println("title :'" + gridTitle +
			 * "',"); } else { if (recordLabel != null) { out.println("title :'" +
			 * recordLabel + "管理',"); } }
			 */
			columnAutoFill = (columnAutoFillStr == null)
					|| !("N".equalsIgnoreCase(columnAutoFillStr));
			out.println("applyTo : 'centerTable',");
			...
			out
					.println("Ext.form.Field.prototype.msgTarget = 'side';//统一指定错误信息提示方式");
			if (!isNeedCheck() || this.hasUpdateRight()) {
				out
			...
			// href='/commonWeb/downloadFileAction.do?FILE_ID=\"+attachRealID+\"'>\"+value+\"</a>\";");
			out
					.println("return toLink('/commonWeb/showAttachAction.do?FILE_ID='+attachRealID+'&FILENAME='+value,value);");
			out.println("}");
			...
			out.println("}");
			out
					.println("var gridHeadField=new Ext.form.Hidden({name:'excelHead'});");
			out.println("var form = new Ext.FormPanel({");
			...
			out
					.println(",{xtype:'hidden', name : 'id',fieldLabel:'id'},{xtype:'hidden', name : 'headId',fieldLabel:'headId'},");
			...
			out.println("var win = new Ext.Window({");
			...
			out.println("store.baseParams.limit=recond;");
			...
			out.println("if(currentID==''){alert('请先保存主表数据');return;}");
			out.println("form.form.reset();");
			...
			out.println("store.load({params:{'" + this.headIdParamName
					+ "':currentID}});");
			out.println("}");
			...
			if (otherSearchForm) {
				out
						.println("var win2 = new Ext.Window({layout:'fit',width:"
			...
			if (otherSearchForm) {
				out.println("searForm.form.reset();");
			...
			out.println("   win.hide(); ");
			out
					.println("   var recs = grid.getSelectionModel().getSelections();");
			...
			out.println("var importForm = new Ext.form.FormPanel({ ");
			...
			out.println("    var inputWindow = new Ext.Window({    ");
			...
			out
					.println("var headIdField=document.getElementById('headId');headIdField.value=currentID;");
			...
			out.println("}else{");
			out.println("var rec = new Ext.data.Record(fields);");
			// out.println("store.add(rec);");
			out.println("store.insert(0,rec);");
			out.println("}");
			out.println("}");
			out.println("var obj = {};");
			...
			out.println("	 }else{");
			out.println("	   var value = item.getValue();");
			out.println("       if(item.getXType() == 'combo'){");
			// out
			...
			out
					.println("             value = selItem.get('name'); obj[item.hiddenName] = selItem.get('id');}");
			out.println("        }");
			...
			out.println("var recs = grid.getSelectionModel().getSelections();");
			out.println("var list = [];");
			...
			// out.println("method:'POST',//请求方式");
			// out.println(" success:
			// function(form,action){Ext.MessageBox.alert('提示','导出Excel成功');},
			// ");
			// out.println("failure: function(){Ext.MessageBox.alert('提示',
			// '导出Excel失败');}});");
			out.println("   win.hide(); ");
			out.println("}");
			...
			out.println("win.show();");
			out.println("form.getForm().getEl().dom.action='"
					+ this.convertHref(action) + "action=exportExcel2"
					+ currentParameterSB);
			out.println("form.getForm().getEl().dom.submit()");
			...
			if (otherSearchForm) {
				...
				if (SemWebAppUtils.isNotEmpty(userFiledsString)) {
					String[] fields = userFiledsString.split(",");
					for (int j = 0; j < fields.length; j++) {
						out.println("var tempValue=searForm.getForm().findField('"
								+ fields[j] + "').getEl().dom.realValue;");
						out.println("searForm.getForm().findField('" + fields[j]
								+ "').setValue(tempValue);");
					}
				}
				if (searchFields != null) {
			...
			// head form
			out.println("var mainForm= new Ext.FormPanel({");
			out
					.println("labelSeparator : '：',frame:true,border:true,renderTo:'totalForm',labelAlign:'right',layout:'form',");
			out.println("items:[");
			out.println(headFormString);
			out.println(",{xtype:'hidden', name : 'id',fieldLabel:'id'}");
			out.println("],");
			if (this.isHideHeadSaveButton()) {
				if (SemWebAppUtils.isNotEmpty(this.headCustonButton)) {
					out.println("buttons:[" + headCustonButton);
					out.println("],");
				}
			} else {
				out
						.println("buttons:[{text : '保存',id:'submitMainButton',handler : submitMainForm}");
				if (SemWebAppUtils.isNotEmpty(this.headCustonButton)) {
					out.println("," + headCustonButton);
				}
				out.println("],");
			}
			out
					.println("reader : new Ext.data.JsonReader({totalProperty:'results',root: 'items',id: 'id'}, ");
			out.println("[" + headField + "]");
			out.println(" )");
			out.println("});");
			if (SemWebAppUtils.isNotEmpty(headId)) {

				out.println("refreshAll();");
			}
			out.println("function refreshAll(){");
			out.println("mainForm.form.load({url:'"
					+ this.convertHref(headAction)
					+ "action=list&weblogic_jsession=" + this.rightString+"&id='+currentID});");
			out.println("refresh();");
			out.println("}");
			// head form save
			out.println("function submitMainForm(){");
			if (this.headSubmitContent != null)
				out.println(headSubmitContent);
			out.println("mainForm.form.submit({");
			out.println("clientValidation:true,//进行客户端验证");
			out.println("waitMsg : '正在提交数据请稍后',//提示信息");
			out.println("waitTitle : '提示',//标题");
			out.println("url : '" + this.convertHref(headAction)
					+ "action=save',//请求的url地址");
			out.println("method:'POST',//请求方式");
			out.println("params  :{'weblogic_jsession':'" + this.rightString
					+ "'},");
			out.println("success:function(mainForm,mainAction){//加载成功的处理函数");
			out
					.println("if(typeof(mainAction.result.message) != 'undefined'){");
			out.println("  Ext.Msg.alert('提示',mainAction.result.message);");
			out.println("}else{");
			out.println(" currentID=mainAction.result.id;refreshAll();");
			out.println("Ext.Msg.alert('提示','保存记录成功');");
			out.println("}},");
			out.println("failure:function(mainForm,mainAction){//加载失败的处理函数");
			out
					.println("if(typeof(mainAction.result.message) != 'undefined'){");
			out.println(" Ext.Msg.alert('提示',mainAction.result.message);");
			out.println("}else{");
			out.println("Ext.Msg.alert('提示','保存记录失败');");
			out.println("}");
			out.println("}");
			out.println("});");
			out.println("return currentID;");
			out.println("}");

			out.println("viewport.render(Ext.body);");
			out.println("});");
			out.println("</script>");
			out
					.println("<body><div id='totalForm' style='width:100%; height:100%;'></div>"
							+ "<div id='centerTable' style='width:100%; height:100%;'></div></body>");
		} catch (IOException e) {
	...
	public String getHeadFormString() {
		return headFormString;
	}

	public void setHeadFormString(String headFormString) {
		this.headFormString = headFormString;
	}
	...
	public String getHeadCustonButton() {
		return headCustonButton;
	}

	public void setHeadCustonButton(String headCustonButton) {
		this.headCustonButton = headCustonButton;
	}
	...
	public String getHeadIdParamName() {
		return headIdParamName;
	}

	public void setHeadIdParamName(String headIdParamName) {
		this.headIdParamName = headIdParamName;
	}
