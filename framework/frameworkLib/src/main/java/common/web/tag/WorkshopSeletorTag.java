...
public class WorkshopSeletorTag extends BaseSeletorTag{

	public final static String LINE_URL = "../PPWeb/workshopAction.do?action=list";
	public static Log logger = LogFactory.getLog(WorkshopSeletorTag.class);
	public static Log sysLogger = LogFactory.getLog("sys");

	public int doEndTag() throws JspException {
		if(SemWebAppUtils.isEmpty(fieldName)) fieldName = "workshopField";
		if(SemWebAppUtils.isEmpty(hiddenName)) hiddenName="workshopID";
		if(SemWebAppUtils.isEmpty(name)) name="workshopName";
		if(SemWebAppUtils.isEmpty(fieldLabel)) fieldLabel="车间";
		...
		try {
			...
			out
					.println("blankText : '车间',typeAhead:true,selectOnFocus:true,hiddenName : '"+hiddenName+"',fieldLabel:'"+fieldLabel+"',name : '"+name+"',");
			out
			...
			out
					.println("workshopField.setValue("+initValue+");");
			out.println("}}," );
			}		
