 ...
 * <code>TreeControl</code> and <code>TreeControlNode</code> classes. This
 * tag has the following user-settable attributes:
 * </p>
 * <ul>
 * <li><strong>action</strong> - Hyperlink to which expand/contract actions
 * should be sent, with a string "<code>${node}</code> marking where the node
 * name of the affected node should be included.</li>
 * <li><strong>images</strong> - Name of the directory containing the images
 * for our icons, relative to the page including this tag. If not specified,
 * defaults to "images".</li>
 * <li><strong>scope</strong> - Attribute scope in which the <code>tree</code>
 * attribute is to be found (page, request, session, application). If not
 * specified, the attribute is searched for in all scopes.</li>
 * <li><strong>style</strong> - CSS style <code>class</code> to be applied
 * to be applied to the entire rendered output of the tree control. If not
 * specified, no style class is applied.</li>
 ...
 * applied to the text of any element that is not currently selected. If not
 * specified, no additional style class is applied.</li>
 * <li><strong>tree</strong> - Attribute name under which the
 * <code>TreeControl</code> bean of the tree we are rendering is stored, in
 * the scope specified by the <code>scope</code> attribute. This attribute is
 * required.</li>
 * </ul>
 * 
 ...
 * @version $Revision: 1.1 $ $Date: 2015/09/21 14:34:28 $
 */
...
public class TreeControlTag extends TagSupport {
	...
	private static final String TREE_EXPANDED_URL = "treeExpandedAction.do";
	
	private static final String SCRIPT_PREFIX="JAVASCRIPT:";

	// ------------------------------------------------------------- Properties

	/**
	 * The hyperlink to be used for submitting requests to expand and contract
	 * tree nodes. The placeholder "<code>${name}</code>" will be replaced
	 * by the <code>name</code> property of the current tree node.
	 */
	//protected String sourcePage = null;

	protected String nodeAction = null;

	/**
	 * The name of the directory containing the images for our icons, relative
	 * to the page including this tag.
	 */
	 ...
	 * attribute. Must be "page", "request", "session", or "application" (or
	 * <code>null</code> for an ascending-visibility search).
	 */
	...
	/**
	 * The CSS style <code>class</code> to be applied to the text of selected
	 * nodes.
	 */
	...
	/**
	 * The CSS style <code>class</code> to be applied to the text of
	 * unselected nodes.
	 */
	...
	protected String icon = DEFAULT_ICON;

	// --------------------------------------------------------- Public Methods

	protected String owenFrame = null;

	public String getOwenFrame() {
		return owenFrame;
	}

	public void setOwenFrame(String owenFrame) {
		this.owenFrame = owenFrame;
	}
	 ...
	 * 
	 * @exception JspException
	 *                if no TreeControl instance can be found
	 */
	protected TreeControl getTreeControl() throws JspException {
		...
		if (scope == null)
			...
			treeControl = pageContext
					.getAttribute(tree, PageContext.PAGE_SCOPE);
	 ...
	 * 
	 * @param out
	 *            The <code>JspWriter</code> to which we are writing
	 * @param node
	 *            The <code>TreeControlNode</code> we are currently rendering
	 * @param level
	 *            The indentation level of this node in the tree
	 * @param width
	 *            Total displayable width of the tree
	 * @param last
	...
	protected void render(JspWriter out, TreeControlNode node, int level,
		...
		if ("ROOT-NODE".equalsIgnoreCase(node.getId())
				&& (node.getName() == null)) {
		  ...
		  String hyperlink = null;
		   String encodedNodeName = URLEncoder.encode(node.getId());
			hyperlink =(node.getAction()!=null)?node.getAction():this.nodeAction;
			hyperlink = replace(hyperlink, "${name}", encodedNodeName);
			hyperlink=((HttpServletResponse) pageContext.getResponse())
					.encodeURL(hyperlink);
        if(hyperlink==null&&node.isLeaf()){//无权限，且为叶子，返回。
        	return;
        }
		...
		String updateTreeAction = TREE_EXPANDED_URL + "?tree_session_name="
				+ tree + "&tree=" + node.getId() + "&redirect_url="+owenFrame;
		updateTreeAction = ((HttpServletResponse) pageContext.getResponse())
				.encodeURL(updateTreeAction);
		out.print("    <td>");
		if ((updateTreeAction != null) && !node.isLeaf()) {
			out.print("<a href=\"");
			out.print(updateTreeAction);
			out.print("\">");
		}
		out.print("<img src=\"");
		out.print(images);
		...
		if (node.getName() != null) {
			...
			if (hyperlink != null) {
				if(hyperlink.toUpperCase().startsWith(SCRIPT_PREFIX)){
					out.print("<a onclick=\"");
					out.print(hyperlink);
					out.print("\"");
					out.print(">");
				} else {
					out.print("<a name='menu"+node.getId()+"' href=\"");
					out.print(hyperlink);
					out.print("\"");
				...
				out.print("<a onmouseover=\"javascript:this.style.cursor='hand'\" name='menu"+node.getId()+"' onclick=\"");
				out.print("self.location.href='" + updateTreeAction + "'");
				out.print("\"");
				out.print(">");
				if (labelStyle != null) {
