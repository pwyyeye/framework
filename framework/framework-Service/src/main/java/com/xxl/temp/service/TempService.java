...
public class TempService extends BaseService implements TempRemote{
	...
	public Integer addTempOrder(TempOrderVo vo) throws BaseException, BaseBusinessException {
		...
		try {	
			...
			vo.setId(result);
			String text=vo.getName()+"???"+SemAppUtils.getFullTime(vo.getCreatedate())+"??????????????????"+
			"????????????"+vo.getSize()+", ?????????"+vo.getColor()+", ???"+vo.getNum()+"???????????????"+vo.getAddress()+"???????????????:"+vo.getTel()+"???";
			try {
				jmsTaskRemote.sendMessageByMail("275609395@qq.com", null, "????????????", text, null, null);
			} catch (Exception e) {
				logger.error("?????????????????????"+e);
			}
